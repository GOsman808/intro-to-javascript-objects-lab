...
const starterPokemon = pokemon.find(
  (pokemon) => pokemon.starter === true && pokemon.type === "fire"
);
...
const dragonType = pokemon.find((pokemon) => pokemon.type === 'dragon' && pokemon.hp >= 90);

const highestHp = pokemon.find((pokemon) => pokemon.hp >=200);

const electricType = pokemon.find((pokemon) => pokemon.type === 'electric' && pokemon.hp >=130);

game.party.push(dragonType, highestHp, electricType);
...
const evolution = {
  ...
  game.party.forEach((pokemonInParty, index) => {
    const evolvedNumber = evolution[pokemonInParty.number];
  
    if (evolvedNumber) {
      const evolvedPokemon = pokemon.find(pokemon => pokemon.number === evolvedNumber);
...
game.catchPokemon = function(pokemonObj) {
  ...
  game.catchPokemon(pokemon.find(pokemon => pokemon.name === 'Squirtle'));
...
game.catchPokemon(pokemon.find(pokemon => pokemon.name === 'Sandshrew'));
